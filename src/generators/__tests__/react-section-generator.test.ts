...
describe("generateReactApp", () => {
  it("generates App.tsx with section imports", async () => {
    ...
    const result = await generateReactApp(dsl, { useLLM: false });
  ...
  it("generates App.css with :root variables", async () => {
    const dsl = makeDSL([
  ...
  it("generates section component files", async () => {
    const root = makeNode({ id: "root", children: ["s1", "s2", "s3"] });
  ...
  it("uses className instead of inline styles", async () => {
  ...
  it("renders text nodes as <p> tags", async () => {
  ...
  it("renders image nodes with <img> tag and objectFit", async () => {
  ...
  it("handles flex layout in JSX", async () => {
  ...
  it("generates section names from English names", async () => {
    const root = makeNode({ id: "root", children: ["s1", "s2", "s3"] });
